...
export const MobileMainTicker: React.FC<MobileMainTickerProps> = () => {
  const { selectedTicker, saveChanges, deleteTicker, creator, tickersData } =
    useLossProfitState();
  ...
  useEffect(() => {
    // changes the selectedticker in the form only when there isnt updates
    if (!unsavedChanges) {
      reset(selectedTicker!);
    }
  }, [selectedTicker, reset]);
    ...
    <>
      <div>
        ...
        <h2
          ...
          onClick={() => console.log(tickersData)}
        >

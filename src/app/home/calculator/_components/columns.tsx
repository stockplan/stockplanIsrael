...
export const columns: ColumnDef<Position>[] = [
    ...
    cell: ({ row, column, table }) => {
      ...
      return (
        <div className="flex gap-2 w-24 items-center justify-center">
          ...
          <button
            className={`px-2 py-1 rounded text-white text-xs ${positionType === "sell" ? "bg-red-500" : "bg-gray-400"}`}
  ...
  },
  {
    ...
    header: () => (
      <DataTableColumnHeader tooltipMsg="Enter the number of stock units you want to buy or sell." title="Quantity" />
    ),
    cell: ({ row, column, table }) => {
  ...
  {
    accessorKey: "cost",
    header: () => <DataTableColumnHeader tooltipMsg="Ask Price * Quantity." title="Cost" />,
    cell: ({ row, column, table }) => {
  ...
  },
  {
    ...
    header: () => <DataTableColumnHeader tooltipMsg="(Expected Profit / Quantity) + Ask Price" title="Exit price" />,
    cell: ({ row, column, table }) => {
      const defaultValue = (row.getValue(column.id) as string) || "0"
  ...
  },
  {
    ...
    header: () => <DataTableColumnHeader tooltipMsg="(Expected Profit / Cost) * 100." title="Exp. Profit %" />,
    cell: ({ row, column, table }) => {
  ...
  },
  {
    ...
    header: () => <DataTableColumnHeader tooltipMsg="(Expected Loss / Quantity) + Ask Price" title="Stop Loss" />,
    cell: ({ row, column, table }) => {
      ...
      const handleBlur = () => {
        ...
        const expectedLossPercent = calculateExpectedLossPercent(expectedLoss, cost)
  ...
  },
  {
    ...
    header: () => <DataTableColumnHeader tooltipMsg={`(Expected Loss / Total Cost) * 100.`} title="Exp. Loss %" />,
    cell: ({ row, column, table }) => {
      const initialValue = row.getValue(column.id) as string | number
      const quantity = row.getValue(ColumnNames.Quantity) as number
      const askPrice = row.getValue(ColumnNames.AskPrice) as number
      ...
      //prettier-ignore
      const handleBlur = () => {
        const lossPercentValue = Number(+lossPercent > 0 ? +lossPercent * -1 : +lossPercent)
